...
print(paste0("Number of cells:", nrow(PCs_df)))
...
for (i in 1:nrow(sign_results)){
    ...
    fig_dir = paste0("/share/ScratchGeneral/anncuo/OneK1K/CRM_interaction/Bcells_Bcell_eQTLs/Figures/PC",pc_number,"_gene_by_genotype/")
    ...
    myplot <- ggplot(pc_geno_expr, aes(x=pc_geno_expr[,(as.numeric(pc_number)+5)], y=gene, colour=as.factor(genotype))) + geom_point(alpha=0.3) + 
        stat_smooth(se=F, linetype = 2, aes(group=as.factor(genotype), colour=as.factor(genotype))) + 
        scale_color_canva(palette = "Art history inspired") + theme_classic() + xlab(pc) +
