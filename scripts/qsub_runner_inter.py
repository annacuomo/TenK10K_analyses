...
if __name__ == '__main__':
    ...
    # fvf_filename = inputs_folder+"sex_beta_fvf_Monocyte.csv"
    fvf_filename = inputs_folder+"fvf_Bcell_betas.csv"
    ...
    for j in range(22):
        ...
        chrom = j+1
        if chrom not in [14, 15, 16, 19]: continue # ad hoc in case some chromosomes are already finished
        ...
        for i in range(len(genes)):
            ...
            # py = f"/share/ScratchGeneral/anncuo/jupyter/conda_notebooks/envs/cellregmap_notebook/bin/python estimate_betas_Monocytes_sex_one_gene.py {chrom} {i}"
            py = f"/share/ScratchGeneral/anncuo/jupyter/conda_notebooks/envs/cellregmap_notebook/bin/python estimate_betas_Bcells_noplasma_one_gene.py {chrom} {i}"
            cmd = f"{qsub} \"{py}\" "
            print(cmd)
            os.system(cmd)
